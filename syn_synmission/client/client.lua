...
AddEventHandler("syn:true", function()
	if count == false then
		count = true
	end
end)

RegisterNetEvent("syn:false")
AddEventHandler("syn:false", function()
	if count == true then
		count = false
	end
end)
    ...
    while true do
		...
		for k,v in pairs(Config.MainJob) do
			...
			if betweencoords < 2.2 then
				...
				elseif pressed and not count then
					print(count) 
...
RegisterNetEvent('syn:syn')
AddEventHandler("syn:syn", function(source)
    ...
    local persona = PlayerPedId()
		...
		for i=1, 10 do
               ...
               -- EnableAttributeOverpower(persona, 0, 100, true)
                --EnableAttributeOverpower(persona, 1, 100, false)
                 Citizen.InvokeNative(0xF6A7C08DF2E28B28, persona, 0, 2500.0)
                Citizen.InvokeNative(0xF6A7C08DF2E28B28, persona, 1, 2500.0)
                Citizen.InvokeNative(0x50C803A4CD5932C5, true) --core
                Citizen.InvokeNative(0xD4EE21B7CC7FD350, true) --core
                PlaySoundFrontend("Core_Fill_Up", "Consumption_Sounds", true, 0)
               -- Citizen.InvokeNative(0x675680D089BFA21F, persona, 100.0)
        end
    elseif status == true then
        TriggerEvent("vorp:TipBottom", "You're already having syn", 4000)
    end
    status = false
